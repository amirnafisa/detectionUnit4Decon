...
dataset_path = "./detection_dataset"
resume_file = None
num_epochs = 1
...
def get_world_size():
    if not is_dist_avail_and_initialized():
        return 1
    return torch.dist.get_world_size()
...
def all_gather(data):
    ...
    torch.dist.all_gather(size_list, local_size)
    ...
    torch.dist.all_gather(tensor_list, tensor)
...
dataset = CustomDataset(os.path.join(dataset_path, 'train'), get_transform(train=True))
dataset_test = CustomDataset(os.path.join(dataset_path, 'test'), get_transform(train=False))
...
class CocoEvaluator(object):
    ...
    def prepare_for_coco_segmentation(self, predictions):
        ...
        for original_id, prediction in predictions.items():
            ...
            rles = [
                coco_mask.encode(np.array(mask[0, :, :, np.newaxis], dtype=np.uint8, order="F"))[0]
                for mask in masks
            ]
            for rle in rles:
